...
9. **The stock notification system goes IN ITS ENTIRETY** (owner directive, 2026-07-12; reaffirmed after Phase 1 round 3): Storyworks uses NO game/VORP notification channel of any kind — no full-screen cards, no bottom tips, no right tips, nothing. Every piece of player-facing runtime messaging renders through Storyworks' own branded system (K4). After K4 lands, a VORP `Notify*` call anywhere in the runtime is a defect. Presentation customization is **server owner/dev only** — configured in the config files; players get what the server sets, no per-player toggles. (The right-tip usage in Phase 1 is a temporary, doomed bridge.) **K4 design mockup APPROVED 2026-07-12** with one amendment: no "County Wire" header — objective slips carry only "New Objective" + the seal. Approved baseline: objective slips (parchment telegram), progress ticks (quiet dark chips), mission cards (top-center document header, never full-screen; brass complete / oxblood failed). **Owner directive (same day): K4 ships as the STANDALONE resource `sovereign_notify`** — county-wide, any script uses it via exports (Notify/Objective/Tick/Card); Storyworks hard-depends on it and is its first consumer.

---

## 1. Approved features — V1 Release Candidate (43 features)
...
| E5 | NPC outfit/appearance variation. | ✅ spike S3 PASSED (owner, 2026-07-12) |
