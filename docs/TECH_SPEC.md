...
- ⚠️ **VORP's objective/bottom-tip notification channel (`NotifyObjective` → `vorp:TipBottom`) never rendered in owner testing** (Phase 1 round 2, 2026-07-12) while the right-tip channel (`NotifyRightTip`) worked every time. Rule until K4 ships: ALL runtime messaging uses the right-tip channel, objectives locale-prefixed, next-objective announcements delayed (~1.5s) to avoid same-channel swallowing. K4 (ruling #9) retires stock channels entirely in Phase 2.
