...
**Current phase: CODING — Phase 3 (Task Vocabulary II: Combat & NPC Operations) IN PROGRESS.** Phase 2 gate PASSED 2026-07-13 ("Phase 2 closed"), tagged `v0.2-alpha` — 12 task types, K4 notifications (via sovereign_notify), K1 tracker, K2 blips, dialogue+voice, physical carry/cargo, all owner-verified over three dev-server rounds. Phase 1 gate PASSED 2026-07-12 (`v0.1-alpha`); Phase 0 PASSED same day. Master Feature List LOCKED v1.4 (43 features, rulings #1–9; §2 candidates: sovereign_menus BUILT, sovereign_prompts parked). Two county-wide standalones now exist as siblings: **sovereign_notify** (hard dependency — K4 renderer) and **sovereign_menus** (independent, no Storyworks dependency). Key field rules in TECH_SPEC: never derive direction server-side; ruling #9 — K4 replaces ALL stock notification channels, any VORP Notify* call in the runtime is a defect; per-node state buckets (ctx.nodeState) for tasks. Phase 3 exit gate: combat checklist incl. ugly paths (dying mid-escort, disconnecting mid-defend, cancelling mid-fight) with ZERO orphaned NPCs.
